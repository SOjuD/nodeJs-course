...
import { UserModelAttributes } from '../interfaces/user.types';
import { createUser, editUser, getUserById, getUsersByLogin, removeUser } from "../services/user.methods";
    ...
    router.route(userPath)
        ...
        .get(validator.query(getUserScheme), async (req: ValidatedRequest<IGetUserSchema>, res) => {
            try {
                ...
                if (req.query.id) user = await getUserById(req.query.id, users);
